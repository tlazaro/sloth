...
import com.belfrygames.sloth.exp.memory.unsafe._
import com.belfrygames.sloth.exp.memory.unsafe.UnsafeMath3D._

object UnsafeSphereWorld4 {
  val NUM_SPHERES = 50
  val spheres = Array.fill[UnsafeGLFrame](NUM_SPHERES)(new UnsafeGLFrame)

  val shaderManager = UnsafeGLShaderManager
  val modelViewMatrix = new UnsafeGLMatrixStack
  val projectionMatrix = new UnsafeGLMatrixStack
  val viewFrustum = new UnsafeGLFrustum
  val transformPipeline = new UnsafeGLGeometryTransform
  ...
  // Can't have static inside method like C++
  // Color values
  val vFloorColor = M3DVector4f(0.0f, 1.0f, 0.0f, 1.0f);
  val vTorusColor = M3DVector4f(1.0f, 0.0f, 0.0f, 1.0f);
  val vSphereColor = M3DVector4f(0.0f, 0.0f, 1.0f, 1.0f);
  lazy val rotTimer = new CStopWatch
  lazy val fpsTimer = new CStopWatch
  val mCamera = M3DMatrix44f()
  var count = 0

  val vLightPos = M3DVector4f(0.0f, 10.0f, 5.0f, 1.0f)
  val vLightEyePos = M3DVector4f()
  ...
  def RenderScene() {
    ...
    // Measure time
    count += 1;
    val time = fpsTimer.GetElapsedSeconds();
    if (time >= 1.0) {
      println("FPS:" + (count / time));
      count = 0;
      fpsTimer.Reset();
    }
