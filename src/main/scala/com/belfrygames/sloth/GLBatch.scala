...
class GLBatch extends GLBatchBase {
  ...
  protected var nVertsBuilding = 0
  // Building up vertexes counter (immediate mode emulator)
  protected var nNumVerts = 0
  // Number of verticies in this batch
  protected var nNumTextureUnits = 0 // Number of texture coordinate sets

  protected var bBatchDone = false;
  // Batch has been built

  protected var pVerts: M3DVector3fArray = null
  // Array of vertices
  protected var pNormals: M3DVector3fArray = null
  // Array of normals
  protected var pColors: M3DVector4fArray = null
  // Array of colors
  ...
  private def setupMultiTexCoord2f(texture: Int) {
    // First see if the vertex array buffer has been created...
    if (uiTextureCoordArray(texture) == 0) {
      // Nope, we need to create it
