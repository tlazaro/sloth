...
object Internal {
  ...
  class SFG_State {
	...
	var ForceIconic : Boolean = _ /* New top windows are iconified */
	var UseCurrentContext : Boolean = _ /* New windows share with current */

	var GLDebugSwitch : Boolean = _ /* OpenGL state debugging switch */
	var XSyncSwitch : Boolean = _ /* X11 sync protocol switch */

	var KeyRepeat : Int = GLUT_KEY_REPEAT_ON /* Global key repeat mode. */
	var Modifiers : Int = INVALID_MODIFIERS /* Current ALT/SHIFT/CTRL state */

	var FPSInterval : Int = _ /* Interval between FPS printfs */
	var SwapCount : Int = _ /* Count of glutSwapBuffer calls */
	var SwapTime : Int = _ /* Time of last SwapBuffers */

	var Time : Long = _ /* Time that glutInit was called */
	var Timers : List[Any] = List(null, null) /* The freeglut timer hooks */
	var FreeTimers : List[Any] = List(null, null) /* The unused timer hooks */

	var IdleCallback : () => Unit = _ /* The global idle callback */

	var ActiveMenus : Int = _ /* Num. of currently active menus */
	var MenuStateCallback : (Int) => Unit = _ /* Menu callbacks are global */
	var MenuStatusCallback : (Int, Int, Int) => Unit = _

	var GameModeSize = new SFG_XYUse(640, 480, true) /* Game mode screen's dimensions */
  
	var GameModeDepth : Int = 16 /* The pixel depth for game mode */
	...
	var ExecState : fgExecutionState.Value = GLUT_EXEC_STATE_INIT /* Used for GLUT termination */
	var ProgramName : String = "" /* Name of the invoking program */
	var JoysticksInitialised : Boolean = _ /* Only initialize if application calls for them */
	var InputDevsInitialised : Boolean = _ /* Only initialize if application calls for them */

	var AuxiliaryBufferNumber : Int = 1 /* Number of auxiliary buffers */
	var SampleNumber : Int = 4 /* Number of samples per pixel */

	var MajorVersion : Int = 1 /* Major OpenGL context version */
	var MinorVersion : Int = 0 /* Minor OpenGL context version */
	var ContextFlags : Int = 0 /* OpenGL context flags */
...
//  #endif
	...
	var ScreenWidth : Int = _; /* The screen's width in pixels */
	var ScreenHeight : Int = _; /* The screen's height in pixels */
	var ScreenWidthMM : Int = _; /* The screen's width in milimeters */
