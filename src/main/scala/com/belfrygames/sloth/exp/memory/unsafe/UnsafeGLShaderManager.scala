...
import org.lwjgl.opengl.UnsafeGL20

import org.lwjgl.{BufferChecks, BufferUtils}
import com.belfrygames.sloth.GLTools._
import com.belfrygames.sloth.VarArgs
import org.lwjgl.opengl.{GLContext}

object UnsafeGLShaderManager {
  ...
  def UseStockShader(nShaderID: Int, uniforms: Any*): Int = {
    ...
    var iInteger = 0
    var mvpMatrix: M3DMatrix44f = new M3DMatrix44f(0)
    var pMatrix: M3DMatrix44f = new M3DMatrix44f(0)
    var mvMatrix: M3DMatrix44f = new M3DMatrix44f(0)
    var vColor: M3DVector4f = new M3DVector4f(0)
    var vLightPos: M3DVector4f = new M3DVector4f(0)
    ...
    nShaderID match {
      case GLT_SHADER_FLAT => {
        ...
        mvpMatrix = va.arg
        UnsafeGL20.glUniformMatrix4(iTransform, false, mvpMatrix.address)

        iColor = glGetUniformLocation(uiStockShaders(nShaderID), "vColor")
        vColor = va.arg
        UnsafeGL20.glUniform4(iColor, vColor.address)
      }
      ...
      case GLT_SHADER_DEFAULT_LIGHT => {
        ...
        mvMatrix = va.arg
        UnsafeGL20.glUniformMatrix4(iModelMatrix, false, mvMatrix.address)

        iProjMatrix = glGetUniformLocation(uiStockShaders(nShaderID), "pMatrix")
        pMatrix = va.arg
        UnsafeGL20.glUniformMatrix4(iProjMatrix, false, pMatrix.address)
      ...
      case GLT_SHADER_POINT_LIGHT_DIFF => {
        iModelMatrix = glGetUniformLocation(uiStockShaders(nShaderID), "mvMatrix")
        mvMatrix = va.arg
        //        println("Model " + mvMatrix.toString())
        UnsafeGL20.glUniformMatrix4(iModelMatrix, false, mvMatrix.address)

        iProjMatrix = glGetUniformLocation(uiStockShaders(nShaderID), "pMatrix")
        pMatrix = va.arg
        //        println("Proj " + mvMatrix.toString())
        UnsafeGL20.glUniformMatrix4(iProjMatrix, false, pMatrix.address)

        iLight = glGetUniformLocation(uiStockShaders(nShaderID), "vLightPos")
        vLightPos = va.arg
        //        println("iLight " + vLightPos.toString())
        UnsafeGL20.glUniform3(iLight, vLightPos.address)

        iColor = glGetUniformLocation(uiStockShaders(nShaderID), "vColor")
        vColor = va.arg
        //        println("iColor " + vColor.toString())
        UnsafeGL20.glUniform4(iColor, vColor.address)
      }
      ...
      case GLT_SHADER_SHADED => {
        ...
        iTransform = glGetUniformLocation(uiStockShaders(nShaderID), "mvpMatrix")
        pMatrix = va.arg
        UnsafeGL20.glUniformMatrix4(iTransform, false, pMatrix.address)
      }
