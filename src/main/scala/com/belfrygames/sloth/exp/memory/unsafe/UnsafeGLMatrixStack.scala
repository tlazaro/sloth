...
class UnsafeGLMatrixStack(private val stackDepth: Int = 64) {
  ...
  var pStack = M3DMatrix44f.array(stackDepth)
  ...
  @inline def LoadMatrix(frame: UnsafeGLFrame) {
  ...
  @inline def MultMatrix(frame: UnsafeGLFrame) {
...
//  }
  ...
  def PushMatrix(frame: UnsafeGLFrame) {
    frame.GetMatrix(mTemp);
    PushMatrix(mTemp);
  }

  // Two different ways to get the matrix
  def GetMatrix(): M3DMatrix44f = {
    return pStack(stackPointer);
  }

  def GetMatrix(mMatrix: M3DMatrix44f) {
