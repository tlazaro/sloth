...
object PixBuffs {
  ...
  var screenWidth = 0
  // Desired window or desktop width
  var screenHeight = 0 // Desired window or desktop height

  var bFullScreen = false
  // Request to run full screen
  ...
  def AdvanceBlurTaget() {
    curBlurTarget = ((curBlurTarget + 1) % 6);
  }

  def GetBlurTarget0(): Int = {
    return (1 + ((curBlurTarget + 5) % 6));
  }

  def GetBlurTarget1(): Int = {
    return (1 + ((curBlurTarget + 4) % 6));
  }

  def GetBlurTarget2(): Int = {
    return (1 + ((curBlurTarget + 3) % 6));
  }

  def GetBlurTarget3(): Int = {
    return (1 + ((curBlurTarget + 2) % 6));
  }

  def GetBlurTarget4(): Int = {
    return (1 + ((curBlurTarget + 1) % 6));
  }

  def GetBlurTarget5(): Int = {
    return (1 + ((curBlurTarget) % 6));
  }

  var iFrames = 0;
  // Frame count
  lazy val frameTimer = new CStopWatch
  ...
  def SetupRC() {
    ...
    pixelDataSize = screenWidth * screenHeight * 3 * 4;
    // XXX This should be unsigned byte
